...
describe("shared Matrix client generations", () => {
  ...
  it("lets a monitor task release its transient lease without joining retirement", async () => {
    const client = createMockClient("main");
    createMatrixClientMock.mockResolvedValue(client);
    const auth = authFor("main");
    ...
    const retirement = createMonitorRetirement([]);
    retirement.waitForTasks.mockImplementation(async () => {
      await transient.release();
    });
    monitor.registerMonitorRetirement(retirement);

    await expect(monitor.release({ mode: "persist" })).resolves.toBeUndefined();
    expect(client.stopAndPersist).toHaveBeenCalledTimes(1);
  });

  it("lets a monitor task release its transient lease during forced retirement", async () => {
    const client = createMockClient("main");
    createMatrixClientMock.mockResolvedValue(client);
    const auth = authFor("main");
    const monitor = await acquireSharedMatrixClient({ auth, role: "monitor", startClient: false });
  ...
  it("fences a timed-out startup until late completion is discarded", async () => {
    vi.useFakeTimers();
    const start = createDeferred();
    const firstClient = createMockClient("first");
    const replacementClient = createMockClient("replacement");
  ...
  it("retains the underlying startup lock after its initiating waiter aborts", async () => {
    const client = createMockClient("main");
    const start = createDeferred();
    client.start.mockReturnValue(start.promise);
    createMatrixClientMock.mockResolvedValue(client);
    const auth = authFor("main");
    const initiator = await acquireSharedMatrixClient({ auth, startClient: false });
    const retained = await acquireSharedMatrixClient({ auth, startClient: false });
    const abortController = new AbortController();

    const initiatingStart = initiator.start(abortController.signal);
    await vi.waitFor(() => expect(client.start).toHaveBeenCalledTimes(1));
    abortController.abort();
    await expectMatrixStartupAbort(initiatingStart);
