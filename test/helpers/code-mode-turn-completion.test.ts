...
describe("Code Mode current-turn completion observer", () => {
  ...
  it("reports only the latest four structural rows without retaining transcript content", () => {
    const f = fixture();
    ...
    if (observed.status !== "pending") {
      throw new Error("errored exec was certified");
    }
    expect(observed.diagnostics).toEqual({
      ...
      recentRows: [
        { index: 6, role: "other", isError: false, contentBlockCount: 1, currentExecResult: false },
        { index: 7, role: "other", isError: false, contentBlockCount: 1, currentExecResult: false },
        {
          index: 8,
          role: "assistant",
          isError: false,
