...
describe("subagent orphan recovery — faithful restart path", () => {
  ...
  it("keeps a v2026.9.2 unmarked row deferred and unchanged across cold restores", async () => {
    const now = Date.now();
    ...
    // v2026.9.2 persisted this otherwise-canonical row before ownership policy existed.
    delete releasedRecord.taskOwnershipPolicy;
    ...
    expect(getSubagentRunByChildSessionKey(childSessionKey)?.taskOwnershipPolicy).toBeUndefined();
