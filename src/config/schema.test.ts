...
describe("config schema", () => {
  ...
  it("orders every known tag before normalized custom tags without mutating hints", () => {
    const knownTags = [
      "security",
      "auth",
      "access",
    ...
    const tags = [
      ...knownTags.toReversed().map((tag) => ` ${tag.toUpperCase()} `),
      " Custom ",
      "custom",
      "__proto__",
      "constructor",
    ...
    const hints = { plain: { label: "Plain", tags }, empty: {} };
    const before = structuredClone(hints);
    Object.freeze(tags);
    Object.freeze(hints.plain);
    Object.freeze(hints.empty);
    Object.freeze(hints);

    const tagged = applyDerivedTags(hints);

    expect(tagged.plain?.tags).toEqual([
      ...knownTags,
      "custom",
      "__proto__",
      "constructor",
    ...
    expect(tagged.empty?.tags).toEqual([]);
    expect(hints).toEqual(before);
    expect(tagged).not.toBe(hints);
    expect(tagged.plain).not.toBe(hints.plain);
    expect(tagged.plain?.tags).not.toBe(tags);
    expect(tagged.empty).not.toBe(hints.empty);
    expect(tagged.empty?.tags).not.toBe(tagged.plain?.tags);
    expect(applyDerivedTags(tagged)).toEqual(tagged);
  });

  it.each([
    {
      path: "cloudWorkers",
      expected: ["network", "automation", "custom"],
    },
    {
      path: "SESSION.SHARING.peer",
      expected: ["access", "privacy", "storage", "custom"],
    },
    {
      path: "session.sharing.peer.child",
      expected: ["security", "storage", "advanced", "custom"],
    },
    {
      path: "custom.oauth",
      expected: ["security", "auth", "advanced", "custom"],
    },
  ])("preserves override depth and hint flag precedence for $path", ({ path, expected }) => {
    const tagged = applyDerivedTags({
      [path]: { sensitive: true, advanced: true, tags: [" Custom ", "custom"] },
    });
    expect(tagged[path]?.tags).toEqual(expected);
  });
